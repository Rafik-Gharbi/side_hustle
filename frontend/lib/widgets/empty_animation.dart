...
class EmptyAnimation extends StatelessWidget {
  ...
  final double? height;
  final Widget child;
  final bool expandIfEmpty;

  const EmptyAnimation({required this.child, required this.itemCount, this.height, this.lottieAsset, this.message, super.key, this.expandIfEmpty = false});

  @override
  Widget build(BuildContext context) => itemCount == 0
      ? expandIfEmpty ? Expanded(child: buildEmptyListIndicator()) : buildEmptyListIndicator()
      : child;

  SizedBox buildEmptyListIndicator() {
    return SizedBox(
